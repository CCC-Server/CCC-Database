...
function s.initial_effect(c)
	...
	-- Special Summon condition (Fusion summon or custom procedure)
	...
	-- Custom Special Summon procedure from Extra Deck
	...
	-- (①) Equip self to a "Nemesis Artifact" monster
	...
	-- (②) Negate Spell/Trap and revive self from S/T zone
...
-- Special Summon Procedure using field/SZONE monsters
...
function s.eqop(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc:IsRelateToEffect(e) and tc:IsFaceup() then
		Duel.Equip(tp,c,tc)
		-- Change this card to Equip Spell
		local e1=Effect.CreateEffect(c)
		...
		-- Equip Limit: only to "Nemesis Artifact" monsters
		local e2=Effect.CreateEffect(c)
...
-- (②) Negate Spell/Trap and Special Summon this card from SZONE
...
function s.eq2tg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and s.eq2filter(chkc,tp) end
