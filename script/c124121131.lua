...
function s.initial_effect(c)
	-- ①: 마법 / 함정 카드의 효과가 발동했을 때
	local e1=Effect.CreateEffect(c)
	e1:SetDescription(aux.Stringid(id,0))
	e1:SetCategory(CATEGORY_DECKDES+CATEGORY_REMOVE)
	e1:SetType(EFFECT_TYPE_ACTIVATE)
	e1:SetProperty(EFFECT_FLAG_CARD_TARGET)
	e1:SetCode(EVENT_CHAINING) -- 체인 반응
	e1:SetCondition(s.chcon)   -- 발동 조건
	e1:SetTarget(s.rmtg)
...
function s.rmtg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	-- 변경점: 카테고리 인포의 매수를 2로 변경
	Duel.SetOperationInfo(0,CATEGORY_DECKDES,nil,0,tp,2)
	Duel.SetOperationInfo(0,CATEGORY_REMOVE,g,#g,0,0)
end

-- [① 제외 효과 처리] 덱 위에서 2장 묘지로 보낸 후 대상 제외
function s.rmop(e,tp,eg,ep,ev,re,r,rp)
	-- 변경점: 묘지로 보내는 실제 처리 매수를 2로 변경
