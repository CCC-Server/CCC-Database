...
function s.bttg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	local bc=e:GetHandler():GetBattleTarget()
	if chk==0 then return bc:IsControler(1-tp) and bc:IsAttribute(ATTRIBUTE_DARK) and bc:IsControlerCanBeChanged() and bc:IsFaceup() end
...
function s.metg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	local rc=re:GetHandler()
	if chk==0 then return rc:IsControler(1-tp) and rc:IsAttribute(ATTRIBUTE_DARK) and rc:IsControlerCanBeChanged() and rc:IsFaceup() end
...
function s.chcon(e,tp,eg,ep,ev,re,r,rp)
	local ch=ev-1
	local rc=re:GetHandler()
	if not (re:IsHasType(EFFECT_TYPE_ACTIVATE) and rc:IsArchetype(ARCHETYPE_DARK_SPIRIT_ART) and rp==tp) then return false end
	...
	return ch_player==1-tp and (ch_c:IsAttribute(ATTRIBUTE_DARK) and ch_c:IsControlerCanBeChanged() and ch_c:IsLocation(LOCATION_MZONE) and ch_c:IsControlerCanBeChanged() and ch_c:IsFaceup()) and not Duel.IsExistingTarget(Card.HasFlagEffect,tp,LOCATION_MZONE,0,1,e:GetHandler(),id)
end
