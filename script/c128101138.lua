...
function s.qtg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then
		return Duel.IsExistingMatchingCard(s.fdfilter,tp,LOCATION_ONFIELD,0,1,nil)
			and Duel.IsExistingMatchingCard(s.fdset,tp,LOCATION_DECK,0,1,nil)
	end
end
function s.fdset(c)
	return c:IsSetCard(0xe2) and c:IsType(TYPE_FIELD) and c:IsAbleToActivate()
end
function s.qop(e,tp,eg,ep,ev,re,r,rp)
	...
	if #g>0 and Duel.SendtoGrave(g,REASON_EFFECT)>0 then
		...
		local sg=Duel.SelectMatchingCard(tp,function(c)
			return s.fdset(c) and not c:IsCode(g:GetFirst():GetCode())
