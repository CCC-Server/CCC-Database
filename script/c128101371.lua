--Aero Maneuver - Rolling Scissors
local s,id=GetID()
...
function s.initial_effect(c)
	...
	--------------------------------
	-- (2) 소환 성공시: 즉시 "Aero Maneuver" 1장 일반 소환 → 덱에서 "Aero Maneuver" 몬스터 1장 묘지로
	--------------------------------
...
function s.nstg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then
		return Duel.IsExistingMatchingCard(s.nsfilter,tp,LOCATION_HAND,0,1,nil)
			and Duel.IsExistingMatchingCard(s.gyfilter,tp,LOCATION_DECK,0,1,nil)
	end
...
function s.nsop(e,tp,eg,ep,ev,re,r,rp)
	-- 즉시 "Aero Maneuver" 몬스터 1장 일반 소환
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SUMMON)
	local g=Duel.SelectMatchingCard(tp,s.nsfilter,tp,LOCATION_HAND,0,1,1,nil)
	if #g>0 then
		local tc=g:GetFirst()
		-- true 로 해서 이미 일반 소환을 쓴 턴에도 추가 일반 소환이 가능하게 처리
		Duel.Summon(tp,tc,true,nil)
	end

	-- 그 후, 덱에서 "Aero Maneuver" 몬스터 1장 묘지로 보냄
	Duel.BreakEffect()
...
--------------------------------
-- (3) 파괴 + "Fight Call" 회수
-- 자신 필드의 마/함 1장 파괴하고, 그 후에 묘지의 "Fight Call" 1장을 패로
--------------------------------
...
function s.destg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chk==0 then
		return Duel.IsExistingTarget(s.stfilter,tp,LOCATION_SZONE,0,1,nil)
			and Duel.IsExistingMatchingCard(s.fcfilter,tp,LOCATION_GRAVE,0,1,nil)
	end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_DESTROY)
	local g=Duel.SelectTarget(tp,s.stfilter,tp,LOCATION_SZONE,0,1,1,nil)
...
function s.desop(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc and tc:IsRelateToEffect(e)
		...
		-- 파괴에 성공했다면, 묘지의 "Fight Call" 1장 패로
		if not Duel.IsExistingMatchingCard(s.fcfilter,tp,LOCATION_GRAVE,0,1,nil) then return end
		Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_ATOHAND)
		local g=Duel.SelectMatchingCard(tp,aux.NecroValleyFilter(s.fcfilter),tp,LOCATION_GRAVE,0,1,1,nil)
		if #g>0 then
