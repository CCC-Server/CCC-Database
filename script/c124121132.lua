...
-- [① 효과 처리] 칸나호크 구조 적용 (1장만 남아도 처리 진행)
function s.tdop(e,tp,eg,ep,ev,re,r,rp)
	-- 1. 자신의 덱 맨 위의 카드를 묘지로 보낸다 (성공해야 후속 진행)
	if Duel.DiscardDeck(tp,1,REASON_EFFECT)<=0 then return end
	
	-- 칸나호크식 긁어오기: Relate 여부만 체크하고 1장이라도 살아있으면 진행
	local tg=Duel.GetTargetCards(e)
	if #tg>0 then
		Duel.BreakEffect()
		
		-- 2. 남은 대상 카드들을 덱으로 되돌리기
		...
		-- 3. [칸나호크식 조건문] 2장이 아니라, 되돌아간 카드가 최소 1장이라도 있다면 드로우 진행
