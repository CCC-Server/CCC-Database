...
s.listed_names={100000652} -- "연금수-주석의 아에토스" 카드 코드

-- Filter for "연금수-주석의 아에토스"
function s.spfilter1(c,e,tp)
    return c:IsCode(100000656) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end

-- Filter for other "연금수" monsters
function s.spfilter2(c,e,tp)
    return c:IsSetCard(0x501) and c:IsCanBeSpecialSummoned(e,0,tp,false,false) and not c:IsCode(100000656)
end
...
function s.drawtg(e,tp,eg,ep,ev,re,r,rp,chk)
    if chk==0 then return Duel.GetFlagEffect(tp,id+1)==0 and Duel.IsPlayerCanDraw(tp,1) end
