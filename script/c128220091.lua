--ANIMALITY 
local s,id=GetID()
function c128220091.initial_effect(c)
		--Activate
...
function s.lkfilter(c,e,tp)
	return c:IsFaceup() and Duel.IsExistingMatchingCard(s.spfilter,tp,LOCATION_DECK,0,1,nil,e,tp,c:GetRace())
end
function s.spfilter(c,e,tp,race)
	return c:IsSetCard(0xc24) and c:IsMonster() and not c:IsRace(race) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
...
function s.spop(e,tp,eg,ep,ev,re,r,rp)
	...
	local g=Duel.SelectMatchingCard(tp,aux.NecroValleyFilter(s.spfilter),tp,LOCATION_DECK,0,1,1,nil,e,tp,tc:GetRace())
	if #g>0 then
