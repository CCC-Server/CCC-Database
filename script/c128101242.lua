...
function s.initial_effect(c)
  ...
  -- ① 싱크로 소환 성공시: 주사위 2개 합계에 따라 덱에서 세트
  ...
  -- ③ (퀵) 자신/상대 턴: 이 카드를 엑스트라로 되돌리고, GY의 "홀로판타지" 2장 특소 → 선택적으로 필드 1장 파괴
...
--------------------------------
-- ① 싱크로 소환 성공시 주사위 2개 → 합계별 세트
-- ● 12 이하: 덱에서 "홀로판타지" 마법/함정 1장 세트
-- ● 13 이상: 덱에서 카운터 함정 1장 세트
--------------------------------
...
function s.dicereplace(e,tp,eg,ep,ev,re,r,rp)
  if rp~=tp then return end                    -- 자신이 굴린 주사위만
  ...
  Duel.RegisterFlagEffect(tp,id,0,0,1)         -- 듀얼 중 1번
end
...
function s.spcost(e,tp,eg,ep,ev,re,r,rp,chk)
  ...
  Duel.SendtoDeck(c,nil,SEQ_DECKTOP,REASON_COST) -- 엑스트라로 반환(표준 처리)
end
