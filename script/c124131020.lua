...
function s.initial_effect(c)
	...
	local e1=Ritual.AddProcGreater({
		handler=c,
		filter=aux.FilterBoolFunction(Card.IsSetCard,0xdd),
		stage2=s.stage2
	})
...
function s.stage2(mat,e,tp,eg,ep,ev,re,r,rp,tc)
	...
	local eg=Duel.GetMatchingGroup(Card.IsType,tp,LOCATION_GRAVE,0,nil,TYPE_TUNER)
	...
	if des>0 then
		Duel.Damage(1-tp,des*300,REASON_EFFECT)
	end
end