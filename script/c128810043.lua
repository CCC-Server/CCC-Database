...
-- 지속 마법 필터
function s.ctspell(c)
	return c:IsSetCard(0xc02) and c:IsType(TYPE_SPELL+TYPE_CONTINUOUS)
end
...
function s.settg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then 
		return Duel.GetLocationCount(tp,LOCATION_SZONE)>0
			and Duel.IsExistingMatchingCard(s.ctspell,tp,LOCATION_DECK,0,1,nil)
	end
end
...
function s.setop(e,tp,eg,ep,ev,re,r,rp)
	...
	local g=Duel.SelectMatchingCard(tp,s.ctspell,tp,LOCATION_DECK,0,1,1,nil)
	local tc=g:GetFirst()
	if tc then
