...
function s.filter1(c,e,tp)
	...
	return (#pg<=0 or (#pg==1 and pg:IsContains(c))) and c:IsFaceup() and c:IsSetCard(0xc01) and (rk>0 or c:IsStatus(STATUS_NO_LEVEL))
...
function s.cfilter(c,tp)
	return c:IsSummonPlayer(tp) and c:IsSetCard(0xc01) and c:IsType(TYPE_XYZ)
end
