...
s.listed_series={SET_SPIRITUAL_WIND_ART} --◆
--Spell/Traps can be activated from the hand or Deck
s.acttg=aux.TargetBoolFunction(Card.IsSetCard,SET_SPIRITUAL_WIND_ART) --◆
function s.actcon(e)
...
function s.cctg1(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then
		return c:IsFaceup()
			...
			and bc:IsFaceup() and bc:IsAttribute(ATTRIBUTE_WIND) --◆
	end
...
function s.cctg2(e,tp,eg,ep,ev,re,r,rp)
	...
	return not (Duel.IsExistingMatchingCard(s.ctfilter,tp,LOCATION_MZONE,LOCATION_MZONE,1,c,c) or c:IsRelateToCard(c))
		...
		and rc:IsFaceup() and rc:IsAttribute(ATTRIBUTE_WIND) --◆
