...
function s.selfspconfilter(c)
	return c:IsFaceup() and (c:IsCode(128101000) or (c:IsSetCard(0xc40) and c:IsLocation(LOCATION_MZONE)))
end
function s.selfspcon(e,tp,eg,ep,ev,re,r,rp)
	return Duel.IsExistingMatchingCard(s.selfspconfilter,tp,LOCATION_ONFIELD,0,1,nil)
end
...
function s.thfilter(c)
	return c:IsSetCard(0xc40) and c:IsSpellTrap() and c:IsAbleToHand()
end
...
function s.threg(e,tp,eg,ep,ev,re,r,rp)
	...
	if not (c:IsLocation(LOCATION_GRAVE) and rc:IsSetCard(0xc40) and r & REASON_FUSION+REASON_LINK ~=0) then return end
