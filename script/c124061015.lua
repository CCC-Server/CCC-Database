...
function s.bttg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	local bc=e:GetHandler():GetBattleTarget()
	if chk==0 then return bc:IsControler(1-tp) and bc:IsAttribute(ATTRIBUTE_FIRE) and bc:IsControlerCanBeChanged() and bc:IsFaceup() end
...
function s.metg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	local rc=re:GetHandler()
	if chk==0 then return rc:IsControler(1-tp) and rc:IsAttribute(ATTRIBUTE_FIRE) and rc:IsControlerCanBeChanged() and rc:IsFaceup() end
...
function s.chcon(e,tp,eg,ep,ev,re,r,rp)
	local ch=ev-1
	local rc=re:GetHandler()
	if not (re:IsHasType(EFFECT_TYPE_ACTIVATE) and rc:IsSetCard(0x514d) and rp==tp) then return false end
