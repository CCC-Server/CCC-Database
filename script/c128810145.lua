...
function s.initial_effect(c)
	-- Xyz Summon Procedure (Light, Level 8, 2 materials)
	Xyz.AddProcedure(c,aux.FilterBoolFunction(Card.IsAttribute,ATTRIBUTE_LIGHT),8,2)
	c:EnableReviveLimit()

	-- ①: 이 카드의 수비력은, 이 카드의 엑시즈 소재의 수 × 500 올린다.
	...
	-- ②: 자신 필드의 다른 빛 속성 엑시즈 몬스터 1장을 대상으로 하고 발동할 수 있다. 이 카드 및 이 카드의 엑시즈 소재를 전부 그 몬스터의 엑시즈 소재로 한다.
...
-- ① 효과: 엑시즈 소재의 수 × 500만큼 수비력 상승
function s.defval(e,c)
	return c:GetOverlayCount()*500
end

-- ② 타겟: 자신 필드의 다른 빛 속성 엑시즈 몬스터 1장
function s.ovfilter(c,e)
	return c:IsFaceup() and c:IsType(TYPE_XYZ) and c:IsAttribute(ATTRIBUTE_LIGHT) and c:IsCanBeOverlayed() and c~=e:GetHandler()
end
