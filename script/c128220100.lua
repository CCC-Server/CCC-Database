--F로그라이크 더블 래틀 헤드
local s,id=GetID()
function c128220100.initial_effect(c)
        ...
        if sc then
            ...
            if sc:IsMonster() then
                ...
                if Duel.SendtoHand(sc,nil,REASON_EFFECT)>0 and sc:IsLocation(LOCATION_HAND) then
                    ...
                    Duel.ShuffleHand(tp)
                    -- 언데드족이고 특수 소환 가능하면 추가 소환 여부 확인
                    if sc:IsRace(RACE_ZOMBIE) and Duel.GetLocationCount(tp,LOCATION_MZONE)>0 
           ...
           elseif sc:IsTrap() or sc:IsSpell() then
			if Duel.SendtoHand(sc,nil,REASON_EFFECT)>0 and sc:IsLocation(LOCATION_HAND) then
                    Duel.ConfirmCards(1-tp,sc)
                    Duel.ShuffleHand(tp)
                   local params={fusfilter=aux.FilterBoolFunction(Card.IsRace,RACE_ZOMBIE)}
			if Fusion.SummonEffTG(params)(e,tp,eg,ep,ev,re,r,rp,0) and Duel.SelectYesNo(tp,aux.Stringid(id,4)) then
				Duel.BreakEffect()
